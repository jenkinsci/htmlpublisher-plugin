...
-   The generated HTML report wrapper now embeds the HTML Publisher JavaScript
    so that downloaded report archives continue to work when opened locally.
    This changes the Content-Security-Policy requirement for generated wrappers:
    Jenkins instances using a custom policy must allow inline scripts with
    `script-src 'unsafe-inline'`. This is relevant to the wrapper generated by
    the plugin; the Jenkins-served report view continues to load the script
    from the plugin resource.
